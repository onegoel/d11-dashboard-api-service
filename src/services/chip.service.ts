...
const getSeasonPowerupsOverview = async (seasonId: number) => {
  const now = new Date();
...
const deselectPowerup = async (chipPlayId: string) => {
  const now = new Date();
...
export const chipService = {
  getSeasonPowerupsOverview,
  selectPowerupForSeasonMatch,
  deselectPowerup,
};